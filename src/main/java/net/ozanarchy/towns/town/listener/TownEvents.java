...
public class TownEvents implements Listener {
    ...
    public TownEvents(DatabaseHandler data, PermissionManager permissionManager, TownsPlugin plugin, EconomyProvider economy, ChunkHandler chunkCache, TownNotifier townNotifier){
        this.db = data;
    ...
    public void setSpawn(Player p) {
        ...
        long cooldown = townConfig.getLong("spawn.set-cooldown-seconds", 120) * 1000;
    ...
    public void spawn(Player p) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                final int delay = townConfig.getInt("spawn.teleport-delay-seconds", 15); // seconds
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            economy.remove(uuid, cost, success -> {
                ...
                Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
                    ...
                    java.util.List<String> blockedWorlds = townConfig.getStringList("claims.unclaimable-worlds");
                    for(String worldName : blockedWorlds){
                    ...
                    upkeepRepository.increaseUpkeep(townId, upkeepConfig.getDouble("payment.costs.claim.added", 10.0));
    ...
    public void removeChunk(Player p){
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            if(success){
                ...
                upkeepRepository.decreaseUpkeep(townId, upkeepConfig.getDouble("payment.costs.claim.refunded", 10.0));
    ...
    public void createTown(Player p, String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
           ...
           if(db.townExists(townName)){
            ...
            economy.remove(uuid, townConfig.getDouble("costs.create", 50.0), success -> {
                if (!success) {
                    p.sendMessage(Utils.getColor(prefix + notEnough));
                    return;
                }
                ...
                try {
                    ...
                    if (plugin.getTownTierService() != null) {
                        plugin.getTownTierService().initializeTownTier(townId);
                    }
                    Bukkit.getScheduler().runTask(plugin, () -> {
    ...
    public void abandonTown(int townId) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            // Always attempt hologram cleanup first in case spawn data is already missing/corrupt.
    ...
    private void updateTownBossBar(Player p, Chunk chunk) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            if (townId != null) {
                ...
                color = BarColor.valueOf(townConfig.getString("bossbar.town.color", "GREEN"));
                style = BarStyle.valueOf(townConfig.getString("bossbar.town.style", "SOLID"));
            } else {
                title = Utils.getColor(messagesConfig.getString("messages.wilderness"));
                color = BarColor.valueOf(townConfig.getString("bossbar.wilderness.color", "WHITE"));
                style = BarStyle.valueOf(townConfig.getString("bossbar.wilderness.style", "SOLID"));
            }
    ...
    private boolean isValidTownName(Player player, String townName) {
        ...
        java.util.List<String> blockedNames = townConfig.getStringList("naming.blacklisted");
